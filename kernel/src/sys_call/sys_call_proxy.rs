...
#[inline(never)]
pub fn shutdown() {
    self::do_sys_call(SystemCallNo::Shutdown, Option::None, Option::None, Option::None);
}
...
fn do_sys_call(eax_sys_nr: SystemCallNo, ebx_opt: Option<u32>, ecx_opt: Option<u32>, edx_opt: Option<u32>) -> u32 {
    let mut res: u32 = 0;
    let sys_nr = eax_sys_nr as u32;
    let ebx = ebx_opt.unwrap_or(0);
    let ecx = ecx_opt.unwrap_or(0);
    let edx = edx_opt.unwrap_or(0);
    ...
    unsafe {
        asm!(
            ...
            in("ecx") ecx,
            in("edx") edx,
            // 输出结果，使用lateout避免寄存器冲突
            lateout("eax") res,
            // 使用lateout告诉编译器这些寄存器可能会被修改
            lateout("ebx") _, 
            lateout("ecx") _, 
            lateout("edx") _, 
        );
